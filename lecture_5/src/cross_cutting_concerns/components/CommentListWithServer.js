...
const CommentListWithServer = ({ requestState, dispatch }) => {
  ...
  // Refresh comments on component mount, empty "dependencies" list means that
  // the action only gets executed once.
  useEffect(refreshCommentsFromServer, []);

  const onCommentDelete = (comment) => {
    server
      .deleteComment({ id: comment.id })
      .then(() => dispatch(commentDeleted(comment)))
      .catch(() => {
        /*
         * How should the component behave if deleting a comments fails?
         */
      });
  };

  const requestButton = (
    <button className="green" onClick={refreshCommentsFromServer}>Refresh comments</button>
  );
    ...
    return (
      <div className="commentList">
        ...
        <CommentList
          ...
          comments={requestState.response}
        />
      </div>
    );
  }
